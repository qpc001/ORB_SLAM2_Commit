...
public:
    ...
    */
    ...
    /** 局部地图点与当前帧特征点的匹配,目的是为了增加局部地图点和当前帧匹配数量
     *  匹配思路: 获取这个局部地图点的描述符, 与当前帧的特征点的描述符进行匹配
     *  1.利用当前帧估计位姿,先将局部地图点投影到当前帧的图像上,作为位置预测,在这个预测坐标的附近进行特征点搜索匹配
     *  2.如果匹配到的某个特征点已经有对应的局部地图点,并且原来的已有的局部地图点被其他关键帧观测到
     *    则跳过这个特征点
    ...
    /**在Tracking里的relocalization中使用
     * 当relocalization前面的步骤匹配到当前帧的mappoint太少的时候,
     * 使用这个函数来增加mappoint(路标点)的匹配
     * 思路: 根据前面优化得到的当前帧的位姿,
     *      将候选关键帧剩下的还没匹配到当前帧的mappoint通过相机位姿\内参,投影到当前帧图像上
     *      再根据描述符搜索匹配的2D点,关联这个mappoint, 增加当前帧mappoint匹配数量
     *
